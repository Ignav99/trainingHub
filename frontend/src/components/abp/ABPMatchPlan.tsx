...
export default function ABPMatchPlan({ partidoId, equipoId }: ABPMatchPlanProps) {
  ...
  // Group assigned by lado
  const ofensivas = assigned.filter(a => a.jugada?.lado === 'ofensivo')
  const defensivas = assigned.filter(a => a.jugada?.lado === 'defensivo')
    ...
    <div className="space-y-3">
      ...
      {assigned.length === 0 ? (
        ...
        <div className="space-y-3">
          ...
          {ofensivas.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-1.5">
                <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-blue-100 text-blue-700">OFENSIVO</span>
                <span className="text-[10px] text-gray-400">{ofensivas.length} jugada{ofensivas.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="space-y-1.5">
                {ofensivas.map(ap => renderAssigned(ap))}
              </div>
            </div>
          )}
          ...
          {defensivas.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-1.5">
                <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-red-100 text-red-700">DEFENSIVO</span>
                <span className="text-[10px] text-gray-400">{defensivas.length} jugada{defensivas.length !== 1 ? 's' : ''}</span>
              </div>
              <div className="space-y-1.5">
                {defensivas.map(ap => renderAssigned(ap))}
              </div>
            </div>
          )}
        </div>
      ...
      {showPicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowPicker(false)}>
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[75vh] overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto max-h-[55vh] p-3 space-y-1.5">
              {available.length === 0 ? (
                <div className="text-center py-8 text-gray-400 text-sm">
                  {library.length === 0 ? 'No hay jugadas en la biblioteca' : 'No hay jugadas disponibles con este filtro'}
                </div>
              ) : (
                available.map(j => {
  ...
  function renderAssigned(ap: ABPPartidoJugada) {
      ...
      <div key={ap.id} className="flex items-center gap-2 p-2 bg-white border border-gray-100 rounded-lg group">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {jugada.codigo && <span className="text-[10px] font-mono text-gray-400">{jugada.codigo}</span>}
        ...
        <button
          ...
          className="p-1 rounded text-gray-300 hover:text-blue-500 opacity-0 group-hover:opacity-100 transition-opacity"
          title="Ver diagrama"
        >
