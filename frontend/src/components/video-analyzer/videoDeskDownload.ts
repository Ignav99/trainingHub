import { extractClipRange, LOCAL_CLIP_MAX_SECONDS } from './extractClip'
...
export async function extractAndDownloadDeskClips(opts: {
  ...
  const { video, kind, clips, buttons, matchLabel, onProgress } = opts
  const buttonById = new Map(buttons.map((b) => [b.id, b]))

  const named = (clip: CodeEvent) => {
    const btn = buttonById.get(clip.buttonId)
    return clipFileName(clipDisplayTitle(clip, btn), clip.startTime, clip.endTime)
  }
