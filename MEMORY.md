...
La pestaña **Convocatoria** abre **Cartel** (JPEG/PDF): todos los convocados por dorsal, sin 11 titular. Citación (hora/lugar) y kit local/visitante editables; camiseta+calzonas+medias de Configuración → Equipaciones, con el escudo del club en el pecho izquierdo (sin recuadro de fondo). Campos `partidos.hora_citacion`, `lugar_citacion`, `kit_convocatoria`. Convocar agrupa **Porteros** primero. El 11 se elige con desplegable en cada puesto (habituales → misma línea → otros), como en partidos condicionados.
