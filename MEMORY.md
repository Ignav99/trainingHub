...
La pestaña **Convocatoria** abre **Cartel** (JPEG/PDF): todos los convocados por dorsal, sin 11 titular. Estadio y árbitro van **separados**; se quita `(F11)` / hierba o césped artificial para dejar solo el nombre del campo (también en citación). **Convocatoria** y jornada van más grandes. El encuentro va a la izquierda; día/hora/lugar/árbitro suben al hueco junto a la ropa (más pequeños). A la derecha, recuadro verde sin fondo «Vestiremos con…». Camiseta, calzonas y medias copian la plantilla del inbox (cuello en V, mangas acampanadas, cinturilla, medias con pie). Equipación se puede mezclar local/visitante. Escudo en el pecho, dentro de la camiseta. `kit_convocatoria` admite `local`, `visitante` o `camiseta:pantalon:medias`. Citación (hora/lugar) editable. Convocar agrupa **Porteros** primero. El 11 se elige con desplegable en cada puesto. **El 11 titular se queda al recargar**: vive en `partidos.notas_pre` (`formacion` + `formacion_slots`) y en `convocatorias.titular`/`posicion_asignada`. Al abrir el partido se rehidrata desde el JSON (aunque llegue como objeto) o, si la lista no trae `notas_pre`, desde los titulares. Guardar no pisa el 11 con un campo vacío.

La pizarra de informe y plan usa el mismo editor que las tareas (`TareaPizarraEditor` / campo entero / animación). Preview compacta + Editar abre el editor a pantalla completa. Vector en `pizarra_diagrama`; JPEG nítido en `pizarra_tactica` para listados. **Exportar PDF** recaptura el SVG real (`data-tactical-pitch`) a 1800px — no el thumbnail 720px ni un icono Lucide.
...
**Comentarios Rival** (Contexto → textarea, `estrategia.notas`) es el olfato del entrenador. Vive en `rivales.scout_manual`, no en intel. `extractPersistentScout` los tiene que guardar; `mergeScoutOnLoad` no los pisa con el plan semanal vacío. Actitud/estilo y dimensiones también van en Contexto y en el PDF.
...
**Contexto RFEF (goles/rachas):** el matching de actas ya no usa subcadenas (`San Vicente` ≠ `San Vicente B`, no se confunde con `mi_equipo_nombre`). GF/GC, racha y goleadores son del rival. **No se mezclan temporadas:** las actas de la temporada pasada se cortan (calendario actual + fecha julio–junio) y se archivan en `rival_intel.historico_temporadas`; el contexto vivo es solo la temporada en curso. **Casa/fuera sale del calendario** (`rfef_jornadas.partidos`: local/visitante + marcador reales). Las actas a menudo ponen al rival como local y sus goles en `goles_local` aunque jugaran fuera (Calavera: 1-3 fuera + 0-0 casa se veía como 4 GF en casa y 100% fuera con 0-0). Los minutos de gol siguen el POV del acta (parciales/alineación) y no se voltean con el calendario. **GF estricto:** liga GF/GC nunca supera la clasificación oficial ni suma un gol extra. La UI usa `stats.pct_victoria` antes que el % de la clasificación para no mezclar 100% victorias con 0 GF. Los minutos (`16'`, `45+2`) se vuelcan aunque falten parciales ofuscados.
