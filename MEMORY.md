...
La pestaña **Convocatoria** abre **Cartel** (JPEG/PDF): todos los convocados por dorsal, sin 11 titular. Estadio y árbitro van **separados** (`partidos.ubicacion` + `partidos.arbitro`); si la RFEF los pega (`Campo…Árbitro: NOMBRE`) se parten y el colegiado sale una sola vez. Kit compacto bajo el escudo del club («Jugaremos de local/visitante» + camiseta). Nombres de convocados grandes. Citación (hora/lugar) y kit editables. Campos `hora_citacion`, `lugar_citacion`, `kit_convocatoria`. Convocar agrupa **Porteros** primero. El 11 se elige con desplegable en cada puesto (habituales → misma línea → otros), como en partidos condicionados.
